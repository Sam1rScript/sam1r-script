...
function renderStats() {
    ...
    if (panelExploits) {
        panelExploits.textContent = allExploits.length;
    }
    if (countLabel) {
...
function renderExploits() {
    ...
    // Фильтруем только Windows эксплойты (можно убрать фильтр, если нужно показывать все)
    var filtered = allExploits.filter(function(e) {
        // Показываем только публичные эксплойты (не hidden и не private)
    ...
    // Сортировка: сначала обновлённые, потом не обновлённые
...
function createExploitHTML(e) {
    var statusText = e.updateStatus ? 'Обновлён' : 'Не обновлён';
    var statusClass = e.updateStatus ? 'status-updated' : 'status-outdated';
    var badgeClass = e.updateStatus ? 'badge-updated' : 'badge-outdated';
    var dotClass = e.updateStatus ? 'dot-updated' : 'dot-outdated';

    var costDisplay = e.cost || (e.free ? 'Free' : 'Неизвестно');
    ...
    return '<div class="exploit-card">' +
        ...
        'Обновлено: ' + e.updatedDate + ' | ' + uncDisplay +
        '</div>' +
        '</div>' +
        '<div class="exploit-status">' +
        '<span class="status-dot ' + dotClass + '"></span>' +
        '<span class="' + statusClass + '">' + statusText + '</span>' +
        '<span class="status-badge ' + badgeClass + '">' + (e.updateStatus ? '✅' : '❌') + '</span>' +
